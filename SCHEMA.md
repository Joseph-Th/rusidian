...
7. `0007_add_versioning` (STATUS task H2): adds version tracking and object_history table
...
| `source` | `source::Source` | `id` (PK) | raw content write-once; `origin` (TEXT, enum), `title`, `raw_content`, `created_at` (RFC3339), `created_by`, `captured_at` (RFC3339), `content_hash`, `ingestion_state` (enum), `version` (INT), `updated_at` (RFC3339) |
| `note` | `note::Note` | `id` (PK) | `title`, `created_at` (RFC3339), `created_by`, `project` (TEXT NULL), `version` (INT), `updated_at` (RFC3339) |
| `block` | `block::Block` | `id` (PK), `note_id` (FK) | `block_type` (enum), `content`, `order` (REAL, fractional for insert-between), `created_at`, `created_by`, `version` (INT), `updated_at` (RFC3339) |
| `entity` | `entity::Entity` | `id` (PK) | `kind` (enum), `name`, `aliases` (JSON), `created_at`, `created_by`, `merged_into` (TEXT NULL, references `entity(id)`), `version` (INT), `updated_at` (RFC3339) |
| `link` | `link::Link` | `id` (PK) | `from_type`/`from_id` (ObjectRef), `to_type`/`to_id` (ObjectRef), `link_type` (enum), `created_at`, `created_by`, `reviewed` (enum), `confidence` (REAL NULL), `version` (INT), `updated_at` (RFC3339) |
| `view` | `view::View` | `id` (PK) | `kind` (enum), `title`, `params` (JSON), `created_at`, `created_by`, `version` (INT), `updated_at` (RFC3339) |
...
| `object_history` | — | `id` (PK) | `entity_type` (TEXT), `entity_id`, `version` (INT), `actor` (JSON), `timestamp` (RFC3339), `operation` (TEXT), `previous_version` (INT NULL), `diff` (JSON NULL); unique(entity_type, entity_id, version) |
